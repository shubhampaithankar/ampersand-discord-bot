          ...
          case `onModule${name}`: {
            ...
            if (name === "Enable") {
              ...
              try {
                ...
                if (buttonComponent) {
                  const row =
                    new ActionRowBuilder<ButtonBuilder>().addComponents(
                      ButtonBuilder.from(
                        buttonComponent as APIButtonComponent,
                      ).setDisabled(true),
                    );
                ...
                console.log(
                  "There was an error in SetMusic command follow-up: ",
                  error,
                );
                await prevInter.editReply({
              ...
              this.collector = inter.channel!.createMessageComponentCollector({
                ...
                time: 1000 * 60 * 15,
                filter: (i) =>
                  i.customId === customId && i.user.id === inter.user.id,
              });
              ...
              if (inter.guildId) {
                try {
                  const updateQuery = { enabled: false };
                  await MusicService.updateMusic(inter.guildId, updateQuery);
                  await prevInter.editReply({
